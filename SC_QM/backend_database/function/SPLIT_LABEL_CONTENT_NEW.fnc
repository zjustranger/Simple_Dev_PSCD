CREATE OR REPLACE FUNCTION PSQM.split_label_content_new (
    p_label      IN     VARCHAR2,
...
IS
    ...
    /******************************************************************************
       ...
       2.0        5/26/2020   CYANG44       2. re-write this function to support lable content add new batch number information.
       ...
       1. the label content is a string, with '|' seperate the content in the label, and also end with '|'. below is an example:
        |P|66666666|Q|100|V|TEST1|N|20200419|S|QT999|B|XM070902-03|M|20200301|D|20200901|
            |p| -> partno: 66666666;
            |Q| -> qty: 100;
            |V| -> supplier: TEST1;
            ...
            |B| -> batch no: XM070902-03
            |M| -> manufaturing date: 20200301 (8 digits yyyymmdd)
            |D| -> due date: 20200901 (8 digits represent the due date incase some supplier only offer due date instead of manufaturing date)
...
BEGIN
    ...
    -- Chauncey add on 20200528 to set all scanning result to upper case.
    v_label := UPPER (p_label);

    SELECT SUBSTR (v_label,
                   INSTR (v_label, '|P|') + 3,
                     INSTR (v_label,
                            '|',
                            INSTR (v_label, '|P|'),
